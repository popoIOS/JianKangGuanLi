    ...
    switch (_lineChartQuadrantType) {
        case JHLineChartQuadrantTypeFirstQuardrant:
        {
            ...
            if (_xLineDataArr.count == 1) {
                _perXLen = self.frame.size.width/2;
            }else
            {
...
- (void)configChartXAndYLength{
    //当x轴只有一个数据时
    _xLength = CGRectGetWidth(self.frame)-self.contentInsets.left-self.contentInsets.right-10;
...
- (void)updateYScale{
        ...
        if (max<=10&&max>5) {
            ...
            for (NSInteger i = 0; i<11; i++) {
                [arr addObject:[NSString stringWithFormat:@"%d",(i+1)*count]];
        ...
        default:{
            if (_valueArr.count) {
                ...
                if (self.isFixedForYArrar) {
                    _yLineDataArr = self.isFixedForYArrar;
                }else{
...
- (void)configChartOrigin{
    ...
    switch (_lineChartQuadrantType) {
        ...
        {
            //当x轴只有一个数据时
            self.chartOrigin = CGPointMake(self.contentInsets.left-10, self.frame.size.height-self.contentInsets.bottom);
        }
            break;
        case JHLineChartQuadrantTypeFirstAndSecondQuardrant:
        {
...
//             _perValue = _perYlen/[[_yLineDataArr firstObject] floatValue];
            _perValue = _yLength/_yLineDataArr.count+1;
...
//                    CGPoint p = P_M(i*_perXLen+self.chartOrigin.x,self.contentInsets.top + _yLength - [valueArr[i] floatValue]*_perValue);
                    ...
                    CGFloat max = [[_yLineDataArr valueForKeyPath:@"@max.floatValue"]  floatValue];
                    CGFloat min = [[_yLineDataArr valueForKeyPath:@"@min.floatValue"] floatValue];

                    
                    CGPoint p = P_M(i*_perXLen+self.chartOrigin.x, _yLength - (_yLength-kXandYSpaceForSuperView-_perValue)/(max-min)*([valueArr[i] floatValue]-min)+self.contentInsets.top-_perValue);
...
- (void)drawAnimation{
    ...
    if (self.isLineVertical == YES) {
        
        NSArray *arr = _drawDataArr[0];
        for (NSInteger i =0; i<arr.count; i++) {
            NSMutableArray *arry = [NSMutableArray array];
            
            for (NSInteger j = 0; j<_drawDataArr.count; j++) {
                NSArray *aa = _drawDataArr[j];
                NSValue *value = aa[i];
                [arry addObject:value];
            }
            if (arry.count != 0) {
                [self drawPathWithDataArr:arry andIndex:i];
...
-(void)getArr:(NSArray *)arr{

    NSMutableArray *arryRed = [NSMutableArray array];
    NSMutableArray *arryYellow = [NSMutableArray array];
    for (NSValue *value in arr) {
        
        CGPoint p = value.CGPointValue;
        
        if (p.y<100) {
            [arryRed addObject:[NSValue valueWithCGPoint:p]];
        }else
        {
            [arryYellow addObject:[NSValue valueWithCGPoint:p]];
        }
    }
...
//    
//    [gradLayer1 setColors :[ NSArray  arrayWithObjects :( id )[[ UIColor  yellowColor ] CGColor ],[( id )[UIColor redColor] CGColor] ,[[UIColor yellowColor] CGColor], nil ]];
//    
//    [gradLayer1 setLocations: @[@0.25, @0.5, @0.75  ]];
//    
//    [gradLayer1 setStartPoint :CGPointMake ( 0 ,  0 )];
//    
//    [gradLayer1 setEndPoint: CGPointMake( 1,  0)];
//    
//    [gradinetLayer addSublayer:gradLayer1];
//    
//    [gradinetLayer setMask:shapeLayer];
//    
//    [self.layer  addSublayer:gradinetLayer];
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(ani.duration * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (weakself.contentFillColorArr.count == weakself.drawDataArr.count) {
            if (self.isLineVertical == YES) {
                shaperLay.fillColor = [[UIColor redColor] CGColor];
...
//                        [pBezier addArcWithCenter:p radius:13 startAngle:0 endAngle:M_PI*2 clockwise:YES];
            ...
            pLayer.strokeColor = color.CGColor;
            CABasicAnimation *ani = [CABasicAnimation animationWithKeyPath:NSStringFromSelector(@selector(strokeEnd))];
