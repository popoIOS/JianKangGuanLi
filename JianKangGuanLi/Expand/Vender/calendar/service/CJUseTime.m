 ...
 *  @return 腊月
 */
- (NSString *) timeChineseCalendarMothWithString:(NSString *)dateStr{

    NSDate *date = [self strToDate:dateStr];
    NSDateComponents *localeComp = [self.chineseCalendar components:NSCalendarUnitYear | NSCalendarUnitMonth |  NSCalendarUnitDay fromDate:date];
    return [self.chineseMonths objectAtIndex:localeComp.month-1];
}
...
-(NSString *)timeChineseMothWithDate:(NSString *)dateStr{
    NSDate *date = [self strToDate:dateStr];
    NSDateComponents *localeComp = [self.chineseCalendar components: NSCalendarUnitMonth fromDate:date];
    
    return [NSString stringWithFormat:@"%ld",localeComp.month];
}
